...
import {
  ...
  CTableHeaderCell, CTableBody, CTableDataCell, CFormInput, CButton, CSpinner,
...
function PreviewContent({ target, onResult }) {
  ...
  useEffect(() => { setOk(null) }, [target?.iconUrl])
  useEffect(() => {
    if (!target?.id) return
    if (ok === null) return
  ...
  return (
    <div>
      <div className="mb-2">
        ...
        <div className="text-secondary" style={{ fontSize: 14 }}>
          링크: <a href={target.url || '#'} target="_blank" rel="noreferrer">{target.url || '-'}</a>
        </div>
        <div className="text-secondary" style={{ fontSize: 14 }}>
...
const SNS = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const norm = (Array.isArray(list) ? list : []).slice(0, MAX)
        .map((x, i) => ({
          id: i + 1,
          title: x.title ?? '',
          iconUrl: x.icon_url ?? '',
  ...
  const handleDelete = useCallback((id) => {
    ...
    setPreviewOk(prev => { const cp = { ...prev }; delete cp[id]; return cp })
  }, [])
  ...
  const resetAll = useCallback(() => {
    setEdits(prev => {
      const cp = { ...prev }
      items.forEach(({ id }) => { if (originals[id]) cp[id] = { ...originals[id] } })
      return cp
    })
    setPreviewOk(prev => {
      const np = { ...prev }
      items.forEach(({ id }) => { np[id] = false })
      return np
    })
    setShowReset(false)
  }, [items, originals])
  ...
  return (
    <>
      ...
      <CModal visible={showSave} onClose={() => setShowSave(false)} backdrop="static" keyboard>
        <CModalHeader><CModalTitle>전체 저장 확인</CModalTitle></CModalHeader>
        <CModalBody>
          <div className="small">
            {rows.map(({ id }) => (
              <div key={`sv-${id}`} className="mb-2">
                <strong>#{id}</strong> — {edits[id]?.title || '(제목없음)'}<br/>
                ...
                <br/>
                링크: <code style={{ wordBreak:'break-all' }}>{edits[id]?.url || '-'}</code> · 순서: {edits[id]?.order ?? 1}
              </div>
            ))}
      ...
      <CModal
        ...
        keyboard
      >
        <CModalHeader><CModalTitle>아이콘 미리보기</CModalTitle></CModalHeader>
        <CModalBody>
          <PreviewContent
            target={preview}
        ...
        <CCard>
          <CCardHeader>
            <div className="d-flex align-items-center justify-content-between" style={{ gap: 12 }}>
              <h2 style={{ fontSize:'1.5rem', fontWeight:'bold', margin:0 }}>SNS 설정 (최대 {MAX}개)</h2>
              <div className="d-flex" style={{ gap: 8 }}>
                ...
                >
                  {saving ? <><CSpinner size="sm" className="me-2" /> 저장중</> : '전체 저장'}
                </CButton>
              </div>
            </div>
          </CCardHeader>
              ...
              <CTableBody>
                {loading ? (
                  ...
                  rows.map(({ id }) => {
                    ...
                    const validOrder = Number.isInteger(e.order) && e.order > 0
                    const previewOK  = previewOk[id] === true
                    const rowValid   = validTitle && validIcon && validUrl && validOrder && previewOK
                    return (
                      <CTableRow key={id}>
                        ...
                        <CTableDataCell className="d-flex align-items-center" style={{ gap: 8 }}>
                          ...
                          {!rowValid && (
                            <span className="text-danger small ms-2">유효하지 않은 값/미리보기 필요</span>
                          )}
                        </CTableDataCell>
                      </CTableRow>
                    )
